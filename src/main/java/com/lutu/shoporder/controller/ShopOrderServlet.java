...
public class ShopOrderServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) { // 來自addShopOrder.jsp
			...
			if (!errorMsgs.isEmpty()) {
				req.setAttribute("sovo", sovo); // 含有輸入格式錯誤的empVO物件,也存入req
				RequestDispatcher failureView = req.getRequestDispatcher("/shopOrder/addShopOrder.jsp");
				failureView.forward(req, res);
				return; // 程式中斷
			}
			...
			/*************************** 2.開始修改資料 *****************************************/
			ShopOrderService sos = new ShopOrderService();
			sovo = sos.addShopOrder(sovo); // 使用 ShopOrderVO 方法

			/*************************** 3.修改完成,準備轉交(Send the Success view) *************/
			req.setAttribute("sovo", sovo); // 資料庫update成功後,正確的的empVO物件,存入req
			String url = "/shopOrder/listAllShopOrder.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url); // 修改成功後,轉交listOneEmp.jsp
			successView.forward(req, res);
